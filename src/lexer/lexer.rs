use crate::file::Span;
use crate::lexer::error::{Error, LexResult};
use crate::lexer::tokens::{Symbol, Token};
use crate::lexer::util::{Radix, convert_to_int, is_whitespace};

macro_rules! get {
    ( $x:expr ) => {
        match $x.peek() {
            Some(c) => c,
            None => return Ok(Token::EOF),
        }
    };
}

pub struct Tokens<'a> {
    input: &'a str,
    pos: usize,
    line: usize,
    column: usize,
}

impl<'a> Tokens<'a> {
    pub fn new(input: &'a str) -> Self {
        Self {
            input,
            pos: 0,
            line: 1,
            column: 1,
        }
    }

    pub fn next(&mut self) -> LexResult<(Token, Span)> {
        self.skip_comments_and_whitespace()?;
        let span = self.pos();
        let token = self.get_token()?;
        Ok((token, span))
    }

    fn get_token(&mut self) -> LexResult<Token> {
        // separators
        if self.accept_sequence("...") {
            return Ok(Symbol::Ellipsis.into());
        }
        if self.accept_sequence("::") {
            return Ok(Symbol::DoubleColon.into());
        }
        let c = get!(self);
        if let Some(t) = match c {
            '(' => Some(Symbol::LeftParen.into()),
            ')' => Some(Symbol::RightParen.into()),
            '{' => Some(Symbol::LeftBrace.into()),
            '}' => Some(Symbol::RightBrace.into()),
            '[' => Some(Symbol::LeftBracket.into()),
            ']' => Some(Symbol::RightBracket.into()),
            ';' => Some(Symbol::Semicolon.into()),
            ',' => Some(Symbol::Comma.into()),
            '.' => Some(Symbol::Dot.into()),
            '@' => Some(Symbol::At.into()),
            _ => None,
        } {
            self.eat();
            return Ok(t);
        }

        if self.accept('\'') {
            if self.accept('\'') {
                return Err((Error::EmptyCharLiteral, self.pos()));
            }
            let c = self.scan_char_literal()?;
            if self.accept('\'') {
                return Ok(Token::CharLiteral(c));
            }
            let column = self.column;
            self.eat_while(
                |tokens| !matches!(tokens.peek(), Some('\r') | Some('\n') | Some('\'')),
                EatMode::IncludeEnd,
            );
            if self.accept('\'') {
                return Err((Error::MultipleCharactersInCharLiteral, Span::new(self.line, column)));
            }
            return Err((Error::UnclosedCharLiteral, Span::new(self.line, column)));
        }
        if self.accept('"') {
            let s = self.scan_string_literal()?;
            if !self.accept('"') {
                return Err((Error::UnclosedStringLiteral, self.pos()));
            }
            return Ok(Token::StringLiteral(s));
        }

        if Self::is_operator_char(&c) {
            return Ok(self.scan_operator().unwrap());
        }

        if c == '0' {
            let numerical_token = match self.peek_next() {
                Some('x') | Some('X') => {
                    self.eat_n(2);
                    self.scan_number(Radix::Hexadecimal)
                }
                Some('b') | Some('B') => {
                    self.eat_n(2);
                    self.scan_number(Radix::Binary)
                }
                _ => self.scan_number(Radix::Octal),
            }?;
            return Ok(numerical_token);
        }
        if matches!(c, '1' | '2' | '3' | '4' | '5' | '6' | '7' | '8' | '9') {
            let numerical_token = self.scan_number(Radix::Decimal)?;
            return Ok(numerical_token);
        }

        if is_identifier_start(c) {
            let identifier_or_kw = self.eat_while(
                |tokens| match tokens.peek() {
                    None => false,
                    Some(c) => is_identifier_part(c),
                },
                EatMode::IncludeEnd,
            );
            let token = match identifier_or_kw {
                // keywords
                "abstract" => Symbol::Abstract.into(),
                "assert" => Symbol::Assert.into(),
                "boolean" => Symbol::Boolean.into(),
                "break" => Symbol::Break.into(),
                "byte" => Symbol::Byte.into(),
                "case" => Symbol::Case.into(),
                "catch" => Symbol::Catch.into(),
                "char" => Symbol::Char.into(),
                "class" => Symbol::Class.into(),
                "const" => Symbol::Const.into(),
                "continue" => Symbol::Continue.into(),
                "default" => Symbol::Default.into(),
                "do" => Symbol::Do.into(),
                "double" => Symbol::Double.into(),
                "else" => Symbol::Else.into(),
                "enum" => Symbol::Enum.into(),
                "extends" => Symbol::Extends.into(),
                "final" => Symbol::Final.into(),
                "finally" => Symbol::Finally.into(),
                "float" => Symbol::Float.into(),
                "for" => Symbol::For.into(),
                "if" => Symbol::If.into(),
                "goto" => Symbol::Goto.into(),
                "implements" => Symbol::Implements.into(),
                "import" => Symbol::Import.into(),
                "instanceof" => Symbol::Instanceof.into(),
                "int" => Symbol::Int.into(),
                "interface" => Symbol::Interface.into(),
                "long" => Symbol::Long.into(),
                "native" => Symbol::Native.into(),
                "new" => Symbol::New.into(),
                "package" => Symbol::Package.into(),
                "private" => Symbol::Private.into(),
                "protected" => Symbol::Protected.into(),
                "public" => Symbol::Public.into(),
                "return" => Symbol::Return.into(),
                "short" => Symbol::Short.into(),
                "static" => Symbol::Static.into(),
                "strictfp" => Symbol::Strictfp.into(),
                "super" => Symbol::Super.into(),
                "switch" => Symbol::Switch.into(),
                "synchronized" => Symbol::Synchronized.into(),
                "this" => Symbol::This.into(),
                "throw" => Symbol::Throw.into(),
                "throws" => Symbol::Throws.into(),
                "transient" => Symbol::Transient.into(),
                "try" => Symbol::Try.into(),
                "void" => Symbol::Void.into(),
                "volatile" => Symbol::Volatile.into(),
                "while" => Symbol::While.into(),

                // literals
                "null" => Symbol::NullLiteral.into(),
                "true" => Token::BooleanLiteral(true),
                "false" => Token::BooleanLiteral(false),

                "_" => Symbol::Underscore.into(),
                // TODO: add contextual keywords
                name => Token::Id(String::from(name)),
            };
            return Ok(token);
        }

        Ok(Token::EOF)
    }

    fn skip_comments_and_whitespace(&mut self) -> LexResult<()> {
        loop {
            let Some(x) = self.peek() else {
                break;
            };
            if is_whitespace(&x) {
                self.skip_whitespace();
            } else if self.accept_sequence("//") {
                self.eat_to_line_end();
            } else if self.accept_sequence("/*") {
                match self.eat_until("*/", EatMode::IncludeEnd) {
                    Err(_) => {
                        return Err((Error::UnterminatedBlockComment, self.pos()));
                    }
                    _ => {}
                };
            } else {
                break;
            }
        }
        Ok(())
    }

    fn scan_char_literal(&mut self) -> LexResult<char> {
        if self.accept('\\') {
            if self.accept('b') {
                return Ok('\u{0008}');
            }
            if self.accept('s') {
                return Ok(' ');
            }
            if self.accept('t') {
                return Ok('\t');
            }
            if self.accept('n') {
                return Ok('\n');
            }
            if self.accept('f') {
                return Ok('\u{000C}');
            }
            if self.accept('r') {
                return Ok('\r');
            }
            if self.accept('"') {
                return Ok('"');
            }
            if self.accept('\'') {
                return Ok('\'');
            }
            if self.accept('\\') {
                return Ok('\\');
            }

            let max_octal_escape_len = match self.peek() {
                Some('0'..='3') => 3,
                _ => 2,
            };
            if let Some((n, len)) =
                Walk::while_holds(&self.input[self.pos..], |c| c.is_digit(Radix::Octal.into()))
                    .take(max_octal_escape_len)
                    .zip(1..)
                    .last()
            {
                let r = u8::from_str_radix(n, 8).unwrap() as char;
                self.eat_n(len);
                return Ok(r);
            }
            return Err((Error::InvalidEscapeSequence, self.pos()));
        }
        match self.eat() {
            Some(c) => Ok(c),
            None => Err((Error::UnclosedCharLiteral, self.pos())),
        }
    }

    fn scan_string_literal(&mut self) -> LexResult<String> {
        let mut s = String::new();
        loop {
            if matches!(self.peek(), Some('"' | '\r' | '\n')) {
                break;
            }
            s.push(self.scan_char_literal()?);
        }

        Ok(s)
    }

    fn scan_number(&mut self, radix: Radix) -> LexResult<Token> {
        let radix: u32 = radix.into();
        match self.peek() {
            Some('_') => {
                return Err((Error::NumericLiteralIllegalUnderscore, self.pos()));
            }
            None => {
                return Err((Error::IncompleteNumericLiteral, self.pos()));
            }
            _ => {}
        }
        let mut trailing_underscore = None;
        let whole = self.eat_while(
            |tokens: &mut Tokens| match tokens.peek() {
                Some(c) if c.is_digit(radix) => {
                    trailing_underscore = None;
                    true
                }
                Some('_') => {
                    trailing_underscore = trailing_underscore.or(Some(tokens.pos()));
                    true
                }
                _ => false,
            },
            EatMode::IncludeEnd,
        );

        if let Some(pos) = trailing_underscore {
            return Err((Error::NumericLiteralIllegalUnderscore, pos));
        }
        let value = convert_to_int(whole, radix).unwrap();
        match self.peek() {
            Some('l') | Some('L') => {
                self.eat();
                Ok(Token::LongLiteral(value))
            }
            _ => Ok(Token::IntegerLiteral(value)),
        }
    }

    fn is_operator_char(c: &char) -> bool {
        matches!(
            c,
            '=' | '>' | '<' | '!' | '~' | '?' | ':' | '&' | '|' | '+' | '-' | '*' | '/' | '^' | '%'
        )
    }

    fn scan_operator(&mut self) -> Option<Token> {
        let walk = Walk::new(&self.input[self.pos..]);

        let (token, len) = walk
            .into_iter()
            .map(|s| Self::take_operator(s))
            .take_while(|t| t.is_some())
            .last()
            .flatten()?;
        self.eat_n(len);
        Some(token)
    }

    fn take_operator(s: &str) -> Option<(Token, usize)> {
        match s {
            "=" => Some((Symbol::Assign.into(), 1)),
            ">" => Some((Symbol::GreaterThan.into(), 1)),
            "<" => Some((Symbol::LessThan.into(), 1)),
            "!" => Some((Symbol::ExclamationMark.into(), 1)),
            "~" => Some((Symbol::Tilde.into(), 1)),
            "?" => Some((Symbol::QuestionMark.into(), 1)),
            ":" => Some((Symbol::Colon.into(), 1)),
            "->" => Some((Symbol::Arrow.into(), 2)),
            "==" => Some((Symbol::Equals.into(), 2)),
            ">=" => Some((Symbol::GreaterThanOrEquals.into(), 2)),
            "<=" => Some((Symbol::LessThanOrEquals.into(), 2)),
            "!=" => Some((Symbol::NotEquals.into(), 2)),
            "&&" => Some((Symbol::LogicalAnd.into(), 2)),
            "||" => Some((Symbol::LogicalOr.into(), 2)),
            "++" => Some((Symbol::Increment.into(), 2)),
            "--" => Some((Symbol::Decrement.into(), 2)),
            "+" => Some((Symbol::Plus.into(), 1)),
            "-" => Some((Symbol::Minus.into(), 1)),
            "*" => Some((Symbol::Multiply.into(), 1)),
            "/" => Some((Symbol::Divide.into(), 1)),
            "&" => Some((Symbol::BitwiseAnd.into(), 1)),
            "|" => Some((Symbol::BitwiseOr.into(), 1)),
            "^" => Some((Symbol::BitwiseXor.into(), 1)),
            "%" => Some((Symbol::Modulo.into(), 1)),
            "<<" => Some((Symbol::LeftShift.into(), 2)),
            ">>" => Some((Symbol::SignedRightShift.into(), 2)),
            ">>>" => Some((Symbol::UnsignedRightShift.into(), 3)),
            "+=" => Some((Symbol::AddAssign.into(), 2)),
            "-=" => Some((Symbol::SubAssign.into(), 2)),
            "*=" => Some((Symbol::MulAssign.into(), 2)),
            "/=" => Some((Symbol::DivAssign.into(), 2)),
            "&=" => Some((Symbol::AndAssign.into(), 2)),
            "|=" => Some((Symbol::OrAssign.into(), 2)),
            "^=" => Some((Symbol::XorAssign.into(), 2)),
            "%=" => Some((Symbol::ModAssign.into(), 2)),
            "<<=" => Some((Symbol::LeftShiftAssign.into(), 3)),
            ">>=" => Some((Symbol::SignedRightShiftAssign.into(), 3)),
            ">>>=" => Some((Symbol::UnsignedRightShiftAssign.into(), 4)),
            _ => None,
        }
    }

    fn skip_whitespace(&mut self) {
        self.eat_while(
            |tokens| {
                if let Some(c) = tokens.peek() {
                    return is_whitespace(&c);
                }
                false
            },
            EatMode::IncludeEnd,
        );
    }

    fn eat_to_line_end(&mut self) {
        self.eat_while(
            |tokens| {
                if let Some(c) = tokens.peek() {
                    return match c {
                        '\r' if tokens.next_is('\n') => true,
                        '\n' | '\r' => false,
                        _ => true,
                    };
                }
                false
            },
            EatMode::IncludeEnd,
        );
    }

    fn next_line(&mut self) {
        self.line += 1;
        self.column = 1;
    }

    fn next_is(&mut self, c: char) -> bool {
        self.peek_next() == Some(c)
    }

    fn peek(&self) -> Option<char> {
        self.peek_n(0)
    }

    fn peek_next(&self) -> Option<char> {
        self.peek_n(1)
    }

    fn peek_n(&self, mut n: usize) -> Option<char> {
        let mut chars = self.input[self.pos..].chars();
        while n > 0 {
            chars.next()?;
            n -= 1;
        }
        chars.next()
    }

    fn accept(&mut self, desired: char) -> bool {
        match self.peek() {
            Some(s) if s == desired => {
                self.eat();
                true
            }
            _ => false,
        }
    }

    fn accept_sequence(&mut self, sequence: &str) -> bool {
        if sequence.is_empty() {
            return true;
        }
        let sequence_len = sequence.chars().count();
        let mut walk = Walk::new(&self.input[self.pos..]);
        match walk.nth(sequence_len - 1) {
            Some(s) if s == sequence => {
                self.eat_n(sequence_len);
                true
            }
            _ => false,
        }
    }

    fn eat(&mut self) -> Option<char> {
        let current = self.peek()?;
        self.column += 1;
        let next_is_lf = self.next_is('\n');
        if current == '\n' || (current == '\r' && !next_is_lf) {
            self.next_line()
        }
        self.pos += current.len_utf8();
        Some(current)
    }

    fn eat_n(&mut self, mut n: usize) -> Option<&str> {
        let mut len = 0;
        while n > 0 {
            len += self.eat()?.len_utf8();
            n -= 1;
        }
        Some(&self.input[self.pos - len..self.pos])
    }

    /// Eat input characters until given sequence is encountered.
    ///
    /// # Arguments
    ///
    /// * `sequence`: the sequence to look for
    /// * `eat_mode`: controls the string returned.</br>
    ///               - `IncludeEnd`: the returned string includes the terminating sequence</br>
    ///               - `NoEnd`: the returned string does not include the terminating sequence</br>
    ///               In any case, the terminating sequence is eaten as well.
    ///
    fn eat_until(&mut self, sequence: &str, eat_mode: EatMode) -> Result<&str, ()> {
        if sequence.is_empty() {
            return Ok(&self.input[self.pos..self.pos]);
        }
        if let Some(i) = self.input[self.pos..].find(&sequence[0..]) {
            let slice_len = i + (if eat_mode.include_end() { sequence.len() } else { 0 });
            let advance_len = i + sequence.len();
            let s = &self.input[self.pos..self.pos + slice_len];
            self.pos += advance_len;
            Ok(s)
        } else {
            Err(())
        }
    }

    fn eat_while<F>(&mut self, mut predicate: F, eat_mode: EatMode) -> &str
    where
        F: FnMut(&mut Self) -> bool,
    {
        let mut last_char = match self.peek() {
            Some(c) => c,
            None => return &"",
        };
        let start = self.pos;
        while let Some(c) = self.peek() {
            last_char = c;
            if !predicate(self) {
                break;
            }
            self.eat();
        }
        let end = if eat_mode.include_end() {
            self.pos
        } else {
            self.pos - last_char.len_utf8()
        };
        &self.input[start..end]
    }

    pub fn pos(&self) -> Span {
        Span::new(self.line, self.column)
    }
}

#[allow(dead_code)]
enum EatMode {
    IncludeEnd,
    NoEnd,
}

impl EatMode {
    pub fn include_end(&self) -> bool {
        match self {
            EatMode::IncludeEnd => true,
            EatMode::NoEnd => false,
        }
    }
}

struct Walk<'a> {
    s: &'a str,
    pos: usize,
    predicate: fn(&char) -> bool,
}

impl<'a> Walk<'a> {
    pub fn new(tokens: &'a str) -> Self {
        Self {
            s: tokens,
            pos: 0,
            predicate: |_c| true,
        }
    }

    pub fn while_holds(tokens: &'a str, predicate: fn(&char) -> bool) -> Self {
        Self { s: tokens, pos: 0, predicate }
    }
}

impl<'a> Iterator for Walk<'a> {
    type Item = &'a str;

    fn next(&mut self) -> Option<Self::Item> {
        let c = &self.s[self.pos..].chars().next()?;
        if !(self.predicate)(&c) {
            return None;
        }
        self.pos += c.len_utf8();
        Some(&self.s[..self.pos])
    }
}

fn is_identifier_start(c: char) -> bool {
    c.is_alphabetic() || c == '_'
}

fn is_identifier_part(c: char) -> bool {
    c.is_alphanumeric() || c == '_'
}
